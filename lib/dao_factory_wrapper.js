...
module.exports = (function(){
    ...
    audit_dao_factory.prototype.createAssociation = function(associatedAuditDAOFactoryWrapper, relationshipCardinality, options, auditOptions){
        ...
        for(var i=0; i<removeFromAssociatedDaoFactory.length;i++){
            ...
            {
                var removedAttributeName = "audit_"+removeFromAssociatedDaoFactory[i];
                delete associatedAuditDAOFactoryWrapper.auditDaoFactory.rawAttributes[removedAttributeName];
                if(associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO && associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO.prototype.attributes && associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO.prototype.attributes instanceof Array){
                    var pos = associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO.prototype.attributes.indexOf(removedAttributeName)
                    if(pos != -1){
                        associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO.prototype.attributes.splice(pos,1)
                    }
        ...
        for(var i=0; i<removeFromThisDaoFactory.length;i++){
            if(configuration.omitFields.indexOf(removeFromThisDaoFactory[i]) === -1){
                var removedAttributeName = "audit_"+removeFromThisDaoFactory[i];
                delete this.auditDaoFactory.rawAttributes[removedAttributeName];
                if(this.auditDaoFactory.DAO && this.auditDaoFactory.DAO.prototype.attributes && this.auditDaoFactory.DAO.prototype.attributes instanceof Array){
                    var pos = this.auditDaoFactory.DAO.prototype.attributes.indexOf(removedAttributeName)
                    if(pos != -1){
                        this.auditDaoFactory.DAO.prototype.attributes.splice(pos,1)
                    }
        ...
        for(var i=0; i<addInThisDaoFactory.length;i++){
            ...
            {
                var addedAttributeName = "audit_"+addInThisDaoFactory[i];
                this.auditDaoFactory.rawAttributes[addedAttributeName] = utils.getDataTypeForDaoFactoryAttribute(this.sequelizeDaoFactory.rawAttributes[addInThisDaoFactory[i]]);
                if(this.auditDaoFactory.DAO && this.auditDaoFactory.DAO.prototype.attributes && this.auditDaoFactory.DAO.prototype.attributes instanceof Array)
                    this.auditDaoFactory.DAO.prototype.attributes.push(addedAttributeName)
            }
        }
        ...
        for(var i=0; i<addInAssociatedDaoFactory.length;i++){
            ...
            {
                var addedAttributeName = "audit_"+addInAssociatedDaoFactory[i];
                associatedAuditDAOFactoryWrapper.auditDaoFactory.rawAttributes[addedAttributeName] = utils.getDataTypeForDaoFactoryAttribute(associatedAuditDAOFactoryWrapper.sequelizeDaoFactory.rawAttributes[addInAssociatedDaoFactory[i]]);
                if(associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO && associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO.prototype && associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO.prototype.attributes  instanceof Array)
                    associatedAuditDAOFactoryWrapper.auditDaoFactory.DAO.prototype.attributes.push(addedAttributeName);
            }
        }
        if(relationshipCardinality == "many"){
